...
if( AKDEBUG ){
	FbimporterHelper::_('include.quickedit');
}
...
if( JVERSION >= 3 ) {
	if ($saveOrder)
	{
		...
		$saveOrderingUrl = 'index.php?option=com_fbimporter&task=formats.'.$method.'&tmpl=component';
...
<table class="table table-striped adminlist" id="itemList">
	...
	<?php foreach ($this->items as $i => $item) :
		...
		$canEdit	= $user->authorise('core.edit',			'com_fbimporter.format.'.$item->a_id);
		$canCheckin	= $user->authorise('core.manage',		'com_fbimporter.format.'.$item->a_id) || $item->a_checked_out == $userId || $item->a_checked_out == 0;
		$canChange	= $user->authorise('core.edit.state',	'com_fbimporter.format.'.$item->a_id) && $canCheckin;
		$canEditOwn = $user->authorise('core.edit.own',		'com_fbimporter.format.'.$item->a_id) && $item->get('c_id') == $userId;
		...
		<?php endif; ?>
			...
			>
				...
				<?php if ($item->get('a_checked_out')) : ?>
					<?php echo JHtml::_('jgrid.checkedout', $i, $item->get('a_checked_out'), $item->get('a_checked_out_time'), 'formats.', $canCheckin); ?>
				<?php endif; ?>
				...
				<?php if ($canEdit || $canEditOwn) : ?>
					<div class="" >
						<a class="quick-edit-content" href="<?php echo JRoute::_('index.php?option=com_fbimporter&task=format.edit&id='.$item->a_id); ?>">
				...
				<div class="pull-left">
					<?php
						...
						if($canEdit || $canEditOwn){
							JHtml::_('dropdown.edit', $item->a_id, 'format.');
						...
						if($canChange || $canEditOwn) {
							if ($item->a_published) :
							JHtml::_('dropdown.unpublish', 'cb' . $i, 'formats.');
							else :
								JHtml::_('dropdown.publish', 'cb' . $i, 'formats.');
							endif;
							JHtml::_('dropdown.divider');
						}
						...
						if ($item->a_checked_out && $canCheckin) :
							JHtml::_('dropdown.checkin', 'cb' . $i, 'formats.');
						endif;
						
						if($canChange || $canEditOwn) {
							if ($trashed) :
								JHtml::_('dropdown.untrash', 'cb' . $i, 'formats.');
							else :
								JHtml::_('dropdown.trash', 'cb' . $i, 'formats.');
							endif;
						}
			...
			<!--PUBLISHED-->
			<td class="center">
				<?php echo JHtml::_('jgrid.published', $item->a_published, $i, 'formats.', $canChange, 'cb', $item->get('a_publish_up'), $item->get('a_publish_down')); ?>
			</td>
